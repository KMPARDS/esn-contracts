...
contract PetTokenConvert {
    ...
    mapping(address => mapping(uint => uint)) public lastWithdrawMonthID;
    mapping(address => mapping(uint => mapping(uint => bool))) public powerBoosterId;
    ...
    function monthlyAnnuity(
        ...
        (uint256 a,uint256 b ,uint256 c ,uint256 d ,uint256 e,uint256 f ) =  pet.pets(_stakerAddress,_petId);
        
        if (msg.sender != _stakerAddress) {
            require(pet.viewNomination(_stakerAddress,_petId,msg.sender), "nomination should be there");
        }
    ...
    function ConvertWES(uint256 _amount) public {
        require(AllowedWES[msg.sender] >= ClaimedWES[msg.sender]+_amount, "You Are not Allowed to these withdraw WES");
