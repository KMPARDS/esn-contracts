...
contract TSGAP {
    ...
    enum DepositStatus { NOTYET_OR_DEFAULTED, GRACE, ONTIME }

    struct SIPPlan {
        bool isPlanActive;
        uint256 minimumMonthlyCommitmentAmount; // minimum amount 500 ES
        uint32 accumulationPeriodMonths; // 12 months
        uint32 benefitPeriodYears; // 9 years
        uint32 gracePeriodSeconds; // 60*60*24*10
        uint32 monthlyBenefitFactor; // this is per 1000; i.e 200 for 20%
        uint32 gracePenaltyFactor; // penalty on first powerBoosterAmount, this is per 1000; i.e 10 for 1%
        uint32 defaultPenaltyFactor; // penalty on first powerBoosterAmount, this is per 1000; i.e 20 for 2%
    }
    ...
    struct SIP {
        ...
        mapping(uint32 => DepositStatus) depositStatus; // 2 => ontime, 1 => grace, 0 => defaulted / not yet
        mapping(uint32 => uint256) monthlyBenefitAmount;
    ...
    uint48 public constant EARTH_SECONDS_IN_MONTH = 2629744;
    ...
    event FundsDeposited(uint256 depositAmount);

    /// @notice event schema for monitoring unallocated fund withdrawn by owner
    event FundsWithdrawn(uint256 withdrawlAmount);

    /// @notice event schema for monitoring new sips by stakers
    event NewSIP(address indexed staker, uint32 sipId, uint256 monthlyCommitmentAmount);

    /// @notice event schema for monitoring deposits made by stakers to sips
    event NewDeposit(
        address indexed staker,
        uint32 indexed sipId,
        uint32 monthId,
        uint256 depositAmount,
        uint256 benefitQueued,
        address depositedBy
    );
    ...
    event NomineeUpdated(
        address indexed staker,
        uint32 indexed sipId,
        address indexed nomineeAddress,
        bool nomineeStatus
    );
    ...
    event AppointeeUpdated(
        address indexed staker,
        uint32 indexed sipId,
        address indexed appointeeAddress,
        bool appointeeStatus
    );
    ...
    modifier meOrNominee(address _stakerAddress, uint32 _sipId) {
        ...
        if (msg.sender != _stakerAddress) {
            require(sip.nominees[msg.sender], "nomination should be there");
        }
        _;
    }

    /// @notice sets up TSGAP contract when deployed
    // /// @param _prepaid: is EraSwap ERC20 Smart Contract Address
    constructor() /*PrepaidEs _prepaid*/
    {
        owner = msg.sender;
        // prepaidEs = _prepaid;
    }
    ...
    function createSIPPlan(
        ...
        uint32 _accumulationPeriodMonths,
        uint32 _benefitPeriodYears,
        uint32 _gracePeriodSeconds,
        uint32 _monthlyBenefitFactor,
        uint32 _gracePenaltyFactor,
        uint32 _defaultPenaltyFactor
    ) public onlyOwner {
    ...
    function addFunds() public payable {
        ...
        require(msg.value > 0, "tokens should be transfered");

        /// @notice increment amount in fundsDeposit
        fundsDeposit = fundsDeposit.add(msg.value);
    ...
    function withdrawFunds(uint256 _withdrawlAmount) public onlyOwner {
        ...
        require(_success, "TSGAP: ES send failing");
    ...
    function _monthlyDeposit(
        ...
        require(
            sip.depositStatus[_monthId] == DepositStatus.NOTYET_OR_DEFAULTED,
        ...
        require(
            fundsDeposit >= _benefitsToBeGiven.add(pendingBenefitAmountOfAllStakers),
            "enough funds should be there in TSGAP"
        );
        ...
        require(
            ...
            "grace period elapsed or too early"
        );
        ...
        //   require(prepaidEs.transferFrom(msg.sender, address(this), _depositAmount), "TSGAP: Prepaid transfer failing");
        //   prepaidEs.transferLiquid(address(this), _depositAmount);
        // } else {
        ...
        sip.depositStatus[_monthId] = _depositStatus;
        sip.monthlyBenefitAmount[_monthId] = _singleMonthBenefit;

        /// @notice adding to total deposit in SIP
        sip.totalDeposited = sip.totalDeposited.add(_depositAmount);
    ...
    function withdrawBenefit(
        ...
        // marking that user has withdrawn upto _withdrawlmonthId month
        uint32 _lastWithdrawlMonthId = sip.lastWithdrawlMonthId;
        sip.lastWithdrawlMonthId = _withdrawlMonthId;
    ...
    /// @param _stakerAddress: address of initiater of this SIP.
    /// @param _sipId: id of SIP in staker address portfolio.
    function withdrawPowerBooster(address _stakerAddress, uint32 _sipId)
        public
        meOrNominee(_stakerAddress, _sipId)
    {
        SIP storage _sip = sips[_stakerAddress][_sipId];

        // taking the next power booster withdrawl
        // not using safemath because this is under safe range
        uint32 _powerBoosterSerial = _sip.powerBoosterWithdrawls + 1;
        ...
        if (msg.sender != _stakerAddress) {
            if (_sip.appointeeVotes > (_sip.numberOfAppointees / 2)) {
        ...
        if (_powerBoosterSerial == 1) {
            ...
            for (uint32 i = 1; i <= sipPlans[_sip.planId].accumulationPeriodMonths; i++) {
                if (_sip.depositStatus[i] == DepositStatus.NOTYET_OR_DEFAULTED) {
                    /// @notice for defaulted months
                    _totalPenaltyFactor += sipPlans[_sip.planId].defaultPenaltyFactor;
                } else if (_sip.depositStatus[i] == DepositStatus.GRACE) {
        ...
        (bool _success, ) = msg.sender.call{ value: _powerBoosterAmount }("");
        require(_success, "TSGAP: ES send failing");
    ...
    function toogleAppointee(
        ...
        if (!_sip.appointees[_appointeeAddress] && _newAppointeeStatus) {
            ...
            _sip.numberOfAppointees = _sip.numberOfAppointees + 1;
            ...
            _sip.numberOfAppointees = uint32(_sip.numberOfAppointees.sub(1));
        }
    ...
    /// @param _stakerAddress: address of initiater of this SIP.
    /// @param _sipId: id of SIP in staker portfolio.
    function appointeeVote(address _stakerAddress, uint32 _sipId) public {
        SIP storage _sip = sips[_stakerAddress][_sipId];
        ...
        _sip.appointeeVotes = _sip.appointeeVotes + 1;
    ...
    function getDepositDoneStatus(
        ...
        uint32 _sipId,
        uint32 _monthId
    ) public view returns (DepositStatus) {
    ...
    function getDepositStatus(
        ...
        require(
            _monthId >= 1 && _monthId <= sipPlans[_sip.planId].accumulationPeriodMonths,
            "invalid deposit month"
        );

        // not using safemath because _monthId is bounded.
        uint48 onTimeTimestamp = _sip.stakingTimestamp + EARTH_SECONDS_IN_MONTH * (_monthId - 1);

        /// @notice deposit allowed only one month before deadline
        if (block.timestamp < onTimeTimestamp - EARTH_SECONDS_IN_MONTH) {
            return DepositStatus.NOTYET_OR_DEFAULTED; /// @notice means deposit is in advance than allowed
        } else if (onTimeTimestamp >= block.timestamp) {
            return DepositStatus.ONTIME; /// @notice means deposit is ontime
        } else if (onTimeTimestamp + sipPlans[_sip.planId].gracePeriodSeconds >= block.timestamp) {
            return DepositStatus.GRACE; /// @notice means deposit is in grace period
        } else {
            return DepositStatus.NOTYET_OR_DEFAULTED; /// @notice means even grace period is elapsed
        }
    }
    ...
    function getPendingWithdrawlAmount(
        ...
        /// @notice restricting early withdrawl
        require(block.timestamp >= withdrawlAllowedTimestamp, "cannot withdraw early");

        // calculate average deposit
        uint256 _benefitToGive;
        for (uint32 _i = _sip.lastWithdrawlMonthId + 1; _i <= _withdrawlMonthId; _i++) {
            uint32 _modulus = _i % sipPlans[_sip.planId].accumulationPeriodMonths;
            if (_modulus == 0) _modulus = sipPlans[_sip.planId].accumulationPeriodMonths;
            _benefitToGive = _benefitToGive.add(_sip.monthlyBenefitAmount[_modulus]);
        }
