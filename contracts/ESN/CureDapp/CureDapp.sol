...
contract CureDapp is EIP1167CloneFactory, RegistryDependent {
    ...
    event NewExpert(address indexed expert,string name,bytes32 indexed specilization,uint256 fees);
    ...
    modifier onlyKycApproved(bytes32 Specilization) {
        require(kycDapp().isKycApproved(msg.sender, 2, 0x435552455f444150500000000000000000000000000000000000000000000000, Specilization) == true, "EXPERT: KYC_REQUIRED");
        _;
    }
    ...
    function newExpert (string memory _Details,string memory name,bytes32 _specilization,uint _fees,uint _incentive) public onlyKycApproved(_specilization)  {
    ...
    function updateExpert (string memory _Details,string memory _name,bytes32 _specilization,uint _fees,uint _incentive) public onlyKycApproved(_specilization) {
    ...
    function BookAppointment(address _expert) public payable{
        ...
        appointments[chatAddress] = Appointment(_expert,msg.sender,msg.value,chatAddress,Status.Pending);
    ...
    function ProceedAppointment(address _chat ) public {
        ...
        // Fees Distribution 
        uint _reward = appointments[_chat].fees.mul(10 + experts[msg.sender].Incentive).div(1000); 
