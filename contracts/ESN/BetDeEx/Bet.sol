...
contract Bet is RegistryDependent {
    ...
    function endBet(uint8 _choice) public {
        ...
        if (_choice < 2 || (_choice == 2 && isDrawPossible)) {
            ...
            require(false, "Bet: CHOICE_NOT_AVAILABLE");
        }
        ...
        betDeEx.emitEndBetEvent(endedBy, _choice, _platformFee);
    }
    ...
    function withdrawPrize(address payable _bettorAddress) public payable onlyKycApproved {
    ...
    function payMoney() public {
        ...
        uint256 _treeAmount = _txAmount.mul(40).div(100); //0.4% of Total 
        uint256 _introducerAmount = _txAmount.mul(40).div(100); // 0.5% of txAmount
        
        /// @dev sending value to a payable function along with giving an argument to the method
        dayswappersContract.payToTree{value: _treeAmount}(_userAddress, [uint256(50), uint256(0), uint256(50)]);
        dayswappersContract.payToIntroducer{value: _introducerAmount}(_userAddress, [uint256(50), uint256(0), uint256(50)]);
